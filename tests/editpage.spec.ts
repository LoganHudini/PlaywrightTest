...
test('Edit Designpage', async({page})=> {
  ...
  await page.locator('tr:has-text("About Leela")').waitFor({ state: 'visible', timeout: 20000 });
  await expect(page.locator('tr:has-text("About Leela")')).toBeVisible();
  await row.click();
  const AboutleelaText = page.locator("//span[@class='page-title']"); 
  await expect(AboutleelaText).toBeVisible();
  
  //Edit Carousel
  await page.locator("(//div[@class='px-2 mt-2 pointer image-details text-underlined'])[1]").waitFor({ state: 'visible' });
  ...
  await page.waitForSelector("//span[text()='Save & Publish']", { timeout: 10000 });
  await page.locator("//span[text()='Save & Publish']").click();
